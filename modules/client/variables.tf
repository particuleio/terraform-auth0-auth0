...
variable "app_type" {
  ...
  validation {
    ...
    error_message = "Valid attributes are [native], [spa], [regular_web], [non_interactive], [sso_integration]"
  }
}
...
variable "token_endpoint_auth_method" {
  ...
  default     = "none"
  validation {
    condition     = contains(["none", "client_secret_post", "client_secret_basic"], var.token_endpoint_auth_method)
    error_message = "Valid attributes are [none], [client_secret_post], [client_secret_basic]"
  }
}
...
variable "organization_usage" {
  ...
  default     = "deny"
  validation {
    condition     = contains(["deny", "allow", "require"], var.organization_usage)
    error_message = "Valid attributes are [deny], [allow], [require]"
  }
}
